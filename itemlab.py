...
hold afterwards:
  ...
  * question IDs stay sequential (MED-Cnn-001 ...) and unique
...
A patch file is a JSON list of operations:
    ...
    {"op": "add",   "unit": "MED-U1-3", "page": 265, "sec": "...", "fmt": "match",
     "q": "...", "opts": ["a","b","c","d"], "ans": 2, "exp": "..."}
    {"op": "edit",  "id": "MED-C1-042", "q": "...", "opts": [...], "ans": 1, "exp": "..."}
...
def chapter_of(op: dict) -> int:
    ...
    match = re.match(r"MED-[UC](\d+)-", str(target))
    if match:
