#!/usr/bin/env python3
"""Validate PULSE Medicine's standalone app and structured chapter source artifacts."""
...
# Total roadmap chapters in the book (Medicine Vol 3, Book p705-1068).
TOTAL_ROADMAP_CHAPTERS = 74
...
LIVE_CHAPTERS = {
    ...
    5: ("Glomerulus - Anatomy", 730, 733, 88, 8),
    6: ("Renal Physiology", 734, 739, 138, 10),
    7: ("Urine Analysis", 740, 747, 140, 10),
    8: ("Basic Approach to Kidney Disease and Renal Artery Stenosis", 748, 752, 83, 8),
    9: ("Thrombotic Microangiopathy", 753, 755, 52, 5),
    10: ("Glomerular Disease - Patterns", 756, 761, 80, 9),
    ...
    52: ('Hyponatremia', 962, 964, 47, 3),
    53: ('Polyuria', 965, 967, 53, 3),
...
def verify() -> None:
    ...
    for question in questions:
        qid = question.get("id", "")
        match = re.fullmatch(r"MED-C(\d+)-(\d{3})", qid)
        if not match:
    ...
    for number, (_title, _first_page, _last_page, _expected_qs, expected_units) in LIVE_CHAPTERS.items():
        ...
        for index, unit in enumerate(chapter_units, 1):
            expected_id = f"MED-U{number}-{index}"
            uid = unit.get("id")
            if uid != expected_id:
